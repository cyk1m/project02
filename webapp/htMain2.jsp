...
// 	});
	...
	//호텔탭 메인 리스트업
		$('#loading').show(); //로딩바를 표시
		$.ajax({ //htMain2.jsp 페이지가 로드될 때 ajax 실행
			url : "mainlist.hotel",
			data : {
				//검색창의 기본 설정 value값을 views/search.jsp(상세페이지 내용)까지 전달 목적 
	...
	// 키워드 검색 후 리스트업 + ajax 로딩
		$('#search').click(function() {
			// 유저가 입력한 키워드, 입실&퇴실일자, 객실수, 인원수 
			...
			gValue = $('#gcount').val()
			$('#loading').show();
			// 검색 버튼 클릭 시 이전 ajax 호출 결과(mainlist)를 숨기고,
			// 다음 ajax 호출 성공 시 보여준다.
			$('#result').hide();
				//목적지를 입력하지 않으면 ajax 통신 안 하도록 처리
				if (keyValue == '') {
					$('#loading').hide();
					$('#result').show(); //이전 ajax 호출 결과(mainlist) 그대로 보여주기
				} else {	
					$.ajax({
					url : "key.hotel",
					data : {
						keyword : keyValue, //dto에 있는 변수로 키를 넣어야 함
						checkin : inValue,
						checkout : outValue,
						stdnum : rValue,
						guestnum : gValue
					...
					success : function(list2) {
						...
						$('#result').show(); //호출 성공시 search.jsp 보이도록
						$('#result').html(list2)
					},
					error : function() {
						alert('실패@@@@')
	...
	//객실수, 투숙인원 카운트
	function rplus() { //객실수+
		$('#rminus').attr('disabled', false) //버튼 활성화
		rValue = $('#rcount').val();
		rVal = parseInt(rValue);
		if (rVal < 5) { //4일 때는 버튼 클릭 시
			$('#rcount').val(rVal + 1); //5개로 바꿀 수 있고
		}else if(rVal >= 5){ //5일 때 버튼 클릭 시엔
			$('#rplus').attr('disabled', true) //버튼 비활성화
		}
		//객실은 객실수=인원일 때 동시에 1씩 증가
		gValue = $('#gcount').val();
	...
	function rminus() { //객실수-
		...
		if (rVal > 1) { //2,3,4,5...
			$('#rcount').val(rVal - 1);
		} else if(rVal == 1){ //인원수 1 아래로 못 내리게
			$('#rminus').attr('disabled', true) //버튼 비활성화
		}
	}
	function gplus() { //인원수+
	...
	function complete() { //카운트 완료버튼 클릭 시
		rValue = $('#rcount').val()
		gValue = $('#gcount').val()
